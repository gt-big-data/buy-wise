...
INSERT INTO products (asin, title, brand, category) VALUES
    ...
    ('B0GR6BVYS5', 'MacBook Neo 13-inch (A18 Pro, 8GB, 256GB)', 'Apple', 'Computers')
...
SELECT product_id, 699.00, NOW() - INTERVAL 30 DAY, TRUE,  FALSE FROM products WHERE asin = 'B0GR6BVYS5' UNION ALL
SELECT product_id, 649.00, NOW() - INTERVAL 14 DAY, TRUE,  TRUE  FROM products WHERE asin = 'B0GR6BVYS5' UNION ALL
SELECT product_id, 699.00, NOW(),                   TRUE,  FALSE FROM products WHERE asin = 'B0GR6BVYS5' UNION ALL
...
SELECT product_id, 'BUY',  93.00,  91.00,  89.99,  0.77 FROM products WHERE asin = 'B07FZ8S74R' UNION ALL
SELECT product_id, 'WAIT', 672.00, 649.00, 639.00, 0.71 FROM products WHERE asin = 'B0GR6BVYS5';
